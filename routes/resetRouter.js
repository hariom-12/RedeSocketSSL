...
router.post("/reset", async (req, res) => {
  ...
  const resetType = req.body.resettype ? req.body.resettype : "Soft";
  const key = uuidv4();
  ...
  let newResopnsFromSocket;
 
  if (resetType == "Soft") {
    let callMessage = `[2,"${key}","Reset",{"type":"Soft"}]`;
    newResopnsFromSocket = broadcast(
      charger,
  ...
  if (resetType == "Hard") {
    let callMessage = `[2,"${key}","Reset",{"type":"Hard"}]`;
    newResopnsFromSocket = broadcast(
      charger,
      req.app.locals.clients,
      callMessage
