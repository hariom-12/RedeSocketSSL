...
router.post("/remotestop", async (req, res) => {
  const charger = req.body.ocpp_cbid ? req.body.ocpp_cbid : null;
  const transactionId = req.body.transactionId ? req.body.transactionId : 0;
  const key = uuidv4();
  ...
  if (transactionId == 0) {
    return res.status(400).json({ result: "transaction ID not found." });
  }

  if (action) {
    const upPayload = {
      key_id: key,
      action_type: "RemoteStopTransaction",
      extra_info: JSON.stringify({ transactionId: transactionId }),
    };
  ...
  const callMessage = `[2,"${key}","RemoteStopTransaction",{"transactionId":${transactionId}}]`;
